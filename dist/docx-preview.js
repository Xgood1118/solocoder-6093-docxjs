...
`;
    ...
    function injectUserStyles(styleContainer, options) {
        if (!options.userStyles || options.userStyles.length === 0)
            return;
        let cssText = "";
    ...
    function cleanupNavigation(bodyContainer, options) {
    ...
    function cleanupSearch(options) {
        const className = options.className;
    ...
    function buildNavigation(bodyContainer, styleContainer, options) {
        const className = options.className;
...
`;
    ...
    function collectHeadings(container, className) {
        const headings = [];
    ...
    function renderOutlineItems(items, parent, className, defaultLevel, currentLevel = 1) {
        for (const item of items) {
            ...
            itemDiv.addEventListener("click", (e) => {
                if (e.target.classList.contains(`${className}-nav-expand`))
                    return;
    ...
    function setupScrollSpy(container, headings, className) {
    ...
    function setupSearch(bodyContainer, styleContainer, options) {
        const className = options.className;
        const searchStyle = `
...
`;
        ...
        function performSearch(query) {
            ...
            contentElements.forEach((el) => {
                if (el.closest(`.${className}-search-container`))
                    return;
                highlightTextInElement(el, regex, className);
            });
    ...
    function exportText(container, options) {
        ...
        if (sections.length === 0) {
            ...
            sections.forEach((section, sectionIndex) => {
                ...
                result += extractTextFromNode(section, className);
            });
        }
        return result.trim();
    }
    function extractTextFromNode(node, className) {
        let text = "";
    ...
    function extractParagraphText(paragraph, className) {
